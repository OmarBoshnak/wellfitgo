...
1.  **Backend Aggregation**: Create a powerful query to fetch all dashboard metrics in one go (or efficiently split).
...
- **Args**: `{ timeRange: '7days' | '30days' | '3months' }`
- **Returns**: A structured object matching the UI needs.
...
1.  **Overview Stats**:
    ...
    *   `checkInRate`: Calculate `(totalMealCompletions / totalScheduledMeals)` for the period.
    *   `responseTime`: (Optional/Placeholder) If message history allows, calc avg time to reply. Otherwise return `null` or a calculated estimate.
...
2.  **Progress Distribution (The Donut Chart)**:
    ...
    *   Return counts and percentages for these 3 buckets.
...
### A. Hook
`src/features/analytics/hooks/useDoctorAnalytics.ts` (Create folder if needed)
- Wrap `useQuery(api.analytics.getDoctorStats, { timeRange })`.
...
- **Loading State**: The screen is data-heavy. Show a clean Skeleton loader or full-screen spinner while fetching.
...
2.  **Date Handling**: Use a library like `date-fns` or native `Date` carefully to respect the `timeRange`.
